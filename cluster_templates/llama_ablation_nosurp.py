...
##################################################
EXPERIMENT_NAME = f"Ex4_LLAMA2-7b_CNN_cofacts_undersample_ablation_nosurp_{time.time()}"
##################################################
PRINTING_FLAG = True
...
class CnnHead(nn.Module):
    def __init__(self, lm_output_size:int, num_classes:int):
        ...
        self.conv1 = nn.Conv1d(in_channels=lm_output_size, out_channels=128, kernel_size=5, padding=2)
        self.act = nn.ReLU()
...
classifier = CnnHead(lm.config.hidden_size, experiment["NUM_CLASSES"]).to(device)
