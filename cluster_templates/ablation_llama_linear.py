...
##################################################
EXPERIMENT_NAME = f"ablation_bothLLAMA2-7b_Linear_liar_{time.time()}"
##################################################
PRINTING_FLAG = True
...
def prepare_dataset (name:str, frac:float, columns:list[str]) -> (object, object, object):
    ...
    if name == "Liar":
        ...
        raw_liar_dataset_train = pd.read_pickle(f"/nethome/phawlitschek/NN-SoftP/pickle_files/train.pkl")
        raw_liar_dataset_validation = pd.read_pickle(f"/nethome/phawlitschek/NN-SoftP/pickle_files/validation.pkl")
        raw_liar_dataset_test = pd.read_pickle(f"/nethome/phawlitschek/NN-SoftP/pickle_files/test.pkl")
...
#    label_mapping = experiment["LABEL_MAPPING"]
    tokens = tokenizer(data["statement"], truncation=True, max_length=2000)
